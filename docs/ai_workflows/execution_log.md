...
- [x] SEC.3 — Certificate pinning for Ktor client — IMPLEMENTED: `CertificatePinning.kt` + `CertificatePinConstants.kt` in `:shared:data` (SEC-02); platform-specific TLS pinning installed in Ktor HttpClient | verified 2026-03-27
  - **2026-03-28 audit — two defects found and fixed (see ADR-011):**
    - **Defect 1 (TODO-SEC02, Production Blocker):** Original implementation used placeholder pins (`sha256/AAAA...`, `sha256/BBBB...`) that would reject every production HTTPS connection. Real pins extracted live from `api.zyntapos.com:443` via VPS OpenSSL. `PRIMARY = sha256/U15ycHcHA6rYMzCokiEnm+i851hmZT+RiFMlagBiKyc=` (leaf, expires 2026-05-31); `BACKUP = sha256/y7xVm0TVJNahMr2sZydE2jQH8SquXV9yLF9seROHHHU=` (LE E7 intermediate). Commit `182ede3`.
    - **Defect 2 (Silent bug):** `installCertificatePinning()` accepted a single `spkiPin: String` — `API_SPKI_PIN_BACKUP` was declared but never passed, making it a no-op. Any leaf cert renewal would have broken all connections. Fixed: signature changed to `vararg spkiPins: String`; both Android (`OkHttp CertificatePinner`) and JVM (`SpkiPinnedTrustManager`) actuals updated to check any pin against any cert in the chain; `buildApiClient()` now passes both pins. Commit `f461873`.
