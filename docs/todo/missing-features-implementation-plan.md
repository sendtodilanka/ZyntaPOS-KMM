...
**Status:** ✅ NEARLY COMPLETE — Remaining deferred items: online ordering API (Phase 3), profit/margin (requires cost-price in sync payload), WebSocket live metrics (Phase 3), blue-green deploy (Phase 3); IRD e-invoice integration deferred to Phase 4; SMS gateway for push notifications planned for Phase 4
...
- Push notification on transfer arrival — SMS gateway integration planned for Phase 4 (FCM removed).
...
- Multi-store operations — ✅ covered (2026-03-28): `WarehouseStockRepositoryTest` (6 nested groups), `ReplenishmentRepositoryTest` (5 groups), `AdminTransferServiceTest` (8 groups, full PENDING→APPROVED→IN_TRANSIT→RECEIVED workflow), `AdminTransferRoutesTest` (route paths + state machine), `AdminReplenishmentRoutesTest` (validation + suggestion logic), `TransferRoutesTest` (POS RBAC: CASHIER forbidden from writes), `ReplenishmentRoutesTest` (POS RBAC + DELETE 404), `AutoReplenishmentJobTest` (filtering, idempotency guard, PO format, notes encoding)
...
- Push notification delivery of JIT token to POS device — SMS gateway planned for Phase 4 (FCM removed)
...
### A5. Analytics & Sentry Integration (TODO-011) — ✅ 100% COMPLETE (ADR-012: Firebase removed 2026-03-28)

**Priority:** P1-HIGH

**What EXISTS:** Full stack analytics and crash reporting wired end-to-end via Kermit + Sentry.
Firebase was implemented on 2026-03-27 then fully removed on 2026-03-28 per ADR-012. See `docs/adr/ADR-012-firebase-removal.md`.

**Completed:**
- [x] ~~Firebase Android SDK in `androidApp/build.gradle.kts` (firebase-bom, analytics-ktx, crashlytics-ktx)~~ — **REMOVED 2026-03-28 (ADR-012)**
- [x] ~~`google-services.json` injection in CI (`_reusable-build-test.yml` step)~~ — **REMOVED 2026-03-28 (ADR-012)**
- [x] ~~`FirebaseAnalytics` initialization in `ZyntaApplication.kt`~~ — **REMOVED 2026-03-28 (ADR-012)**
- [x] `AnalyticsTracker` interface in `:shared:core` (4 methods + event/param constants)
- [x] `AnalyticsService` (commonMain, Kermit-only) in `:shared:data` — replaces Firebase expect/actual
- [x] Koin binding `AnalyticsTracker` + `AnalyticsService` in `DataModule` (common)
- [x] Sentry initialization in Android app (`ZyntaApplication.kt`)
...
> Push notification — SMS gateway integration planned for Phase 4 (FCM removed).
...
**Deferred to Phase 3:**
- [ ] Push notification when transfer arrives at destination store — SMS gateway (Phase 4, FCM removed)
...
**Priority:** PHASE-4 (deferred from Phase 3)
**Module:** `:composeApp:feature:accounting`

> **Status (2026-03-28):** IRD e-invoice integration has been **deferred to Phase 4**.
> All implementation code (EInvoice domain model, EInvoiceRepository, EInvoiceRepositoryImpl,
> IrdApiClient expect/actual, all EInvoice use cases, EInvoiceViewModel/State/Intent/Effect,
> EInvoiceListScreen, EInvoiceDetailScreen, e_invoices.sq) has been **removed from the codebase**
> pending IRD sandbox access and finalisation of the XML invoice format specification.
>
...
> will be re-implemented in Phase 4.

**Phase 4 tasks:**
- [ ] Obtain IRD sandbox access and finalise XML invoice format specification
- [ ] Re-implement `EInvoice` domain model, repository, and use cases
- [ ] Re-implement `IrdApiClient` (expect/actual) with mTLS PKCS12 support
- [ ] Re-implement `EInvoiceListScreen` and `EInvoiceDetailScreen` UI
- [ ] Verify tax calculation against IRD official tax rules
- [ ] Integration tests with IRD sandbox
...
- [x] ~~`google-services.json` decode step~~ — **REMOVED 2026-03-28 (ADR-012)**: Firebase removed; `_reusable-build-test.yml` no longer injects `GOOGLE_SERVICES_JSON`
...
| **IRD e-invoice submission** — implementation code removed; deferred to Phase 4 pending IRD sandbox access and XML format specification | PHASE-4 | Deferred |
...
| ~~**Color-only status indicators**~~ — ✅ DONE (2026-03-25): Icons added to `StockBadge` (ProductCard + ProductListScreen), `InvoiceStatusBadge` (detail), `PurchaseOrderCard` (ReplenishmentScreen) | Stock, Transfer | ~~MEDIUM~~ |
...
| ~~**Inconsistent empty states**~~ — ✅ DONE (2026-03-26): Replaced ad-hoc Text/Column/Icon empty states with `ZyntaEmptyState` design system component across 19 screens: EmployeeList, LeaveManagement, Payroll, ShiftScheduler, Attendance, ExpenseList, ExpenseCategory, CouponList, WarehouseList, StockTransferList, WarehouseRackList, RackProductList, TransitTracker, JournalEntryList, ChartOfAccounts (with CTA), AccountingLedger, AuditLog, Backup, UserManagement | All feature modules | ✅ DONE |
