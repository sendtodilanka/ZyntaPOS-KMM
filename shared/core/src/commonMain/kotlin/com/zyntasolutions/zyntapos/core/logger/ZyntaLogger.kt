 ...
 * ```kotlin
 * class ProductViewModel(private val log: ZyntaLogger) {
 *     fun load() {
 ...
 */
class ZyntaLogger(val defaultTag: String = "ZyntaPOS") {
    ...
    /**
     * Returns a new [ZyntaLogger] whose [defaultTag] is set to [moduleName].
     *
     * ```kotlin
     * private val log = ZyntaLogger.forModule("ProductRepository")
     * ```
     */
    fun withTag(moduleName: String): ZyntaLogger = ZyntaLogger(defaultTag = moduleName)

    companion object {
        /** Convenience factory. Equivalent to `ZyntaLogger(moduleName)`. */
        fun forModule(moduleName: String): ZyntaLogger = ZyntaLogger(defaultTag = moduleName)

        // ── Static convenience methods (Android-style tag-first signature) ───────

        /** Static [Severity.Verbose] — `ZyntaLogger.v(TAG, "msg")` style. */
        fun v(tag: String, message: String, throwable: Throwable? = null) =
            Logger.withTag(tag).v(message, throwable)

        /** Static [Severity.Debug] — `ZyntaLogger.d(TAG, "msg")` style. */
        fun d(tag: String, message: String, throwable: Throwable? = null) =
            Logger.withTag(tag).d(message, throwable)

        /** Static [Severity.Info] — `ZyntaLogger.i(TAG, "msg")` style. */
        fun i(tag: String, message: String, throwable: Throwable? = null) =
            Logger.withTag(tag).i(message, throwable)

        /** Static [Severity.Warn] — `ZyntaLogger.w(TAG, "msg")` style. */
        fun w(tag: String, message: String, throwable: Throwable? = null) =
            Logger.withTag(tag).w(message, throwable)

        /** Static [Severity.Error] — `ZyntaLogger.e(TAG, "msg")` style. */
