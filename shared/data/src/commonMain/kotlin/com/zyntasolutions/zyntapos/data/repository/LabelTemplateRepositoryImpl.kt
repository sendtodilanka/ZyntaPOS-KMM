...
class LabelTemplateRepositoryImpl(
    ...
    override suspend fun save(template: LabelTemplate): Result<Unit> = withContext(Dispatchers.IO) {
        runCatching {
            q.upsertTemplate(
                ...
                show_sale_price        = if (template.showSalePrice) 1L else 0L,
                sale_price_label       = template.salePriceLabel,
                show_expiry_date       = if (template.showExpiryDate) 1L else 0L,
                show_batch_number      = if (template.showBatchNumber) 1L else 0L,
                show_sequential_serial = if (template.showSequentialSerial) 1L else 0L,
            )
        }.fold(
    ...
    private fun toDomain(row: Label_templates) = LabelTemplate(
        ...
        showSalePrice        = row.show_sale_price == 1L,
        salePriceLabel       = row.sale_price_label,
        showExpiryDate       = row.show_expiry_date == 1L,
        showBatchNumber      = row.show_batch_number == 1L,
        showSequentialSerial = row.show_sequential_serial == 1L,
    )
}
