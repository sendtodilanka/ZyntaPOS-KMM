...
class AdminAuthServiceTest {
    ...
    companion object {
        ...
        /**
         * Real instance — safe for these tests because the methods under test
         * (JWT sign/verify) are pure functions that never call auditService.log().
         */
    ...
    fun `issueMfaPendingToken produces valid HS256 JWT with type=admin_mfa_pending`() {
        val service = AdminAuthService(testConfig(), noOpAudit)
    ...
    fun `verifyMfaPendingToken returns null for access-type token (type segregation)`() {
        ...
        val accessTyped = JWT.create()
            ...
            .withClaim("type", "admin_access")   // wrong type
