...
#   • zyntapos_webmail  — Roundcube webmail (auto-migrated by Roundcube on first boot)
#
...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
    ...
    CREATE DATABASE zyntapos_chatwoot OWNER $POSTGRES_USER;
EOSQL

echo "=== Service databases created: zyntapos_api, zyntapos_license, zyntapos_webmail, zyntapos_chatwoot ==="
