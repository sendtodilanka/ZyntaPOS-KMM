...
| **Layout contract** | Applies `ZyntaSpacing.md` horizontal and `ZyntaSpacing.sm` vertical padding — a POS screen layout rule that must not leak into the generic design system. |
...
1. **Never duplicate UI logic.** If you need a new icon, colour, or keyboard behaviour,
   add a parameter to `ZyntaSearchBar` and thread it through here — do not copy the
...
3. **If the wrapper grows beyond ~60 lines**, treat it as a signal that either
   `ZyntaSearchBar` needs a new parameter, or the POS screen needs a decomposition
   review.
