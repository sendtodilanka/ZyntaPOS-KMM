        ...
        viewModel.effects.collect { effect ->
            when (effect) {
                is WarehouseEffect.TransitEventAdded ->
                    snackbar.showSnackbar(s[StringResource.MULTISTORE_TRANSIT_EVENT_LOGGED])
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = {
                    Column {
                        Text(s[StringResource.MULTISTORE_TRANSIT_TRACKER])
                        Text(
                            s[StringResource.MULTISTORE_TRANSFER_ID_FORMAT, transferId.take(8)],
        ...
        floatingActionButton = {
            ...
            if (transfer?.status == StockTransfer.Status.IN_TRANSIT) {
                FloatingActionButton(
                    ...
                    Icon(Icons.Default.Add, contentDescription = s[StringResource.MULTISTORE_LOG_TRANSIT_EVENT_CD])
                }
            }
        },
        ...
        Column(
            ...
            if (state.transitHistory.isEmpty()) {
                ZyntaEmptyState(
                    title = s[StringResource.MULTISTORE_NO_TRANSIT_EVENTS],
                    icon = Icons.Default.LocationOn,
                    subtitle = s[StringResource.MULTISTORE_TAP_LOG_EVENT],
                )
            } else {
...
private fun InTransitCountBanner(count: Int) {
    ...
    Card(
        ...
        Row(
            ...
            Text(
                text = s[StringResource.MULTISTORE_IN_TRANSIT_COUNT_FORMAT, count, if (count != 1) "s" else ""],
...
private fun TransitEventForm(
    ...
    Card(modifier = Modifier.fillMaxWidth()) {
        Column(modifier = Modifier.padding(16.dp)) {
            Text(
                s[StringResource.MULTISTORE_LOG_TRANSIT_EVENT_TITLE],
            ...
            ExposedDropdownMenuBox(
                ...
                OutlinedTextField(
                    ...
                    onValueChange = {},
                    readOnly = true,
                    label = { Text(s[StringResource.MULTISTORE_EVENT_TYPE_LABEL]) },
            ...
            OutlinedTextField(
                ...
                label = { Text(s[StringResource.MULTISTORE_LOCATION_LABEL]) },
                placeholder = { Text(s[StringResource.MULTISTORE_LOCATION_HINT]) },
                modifier = Modifier.fillMaxWidth(),
                singleLine = true,
            )
            ...
            OutlinedTextField(
                ...
                label = { Text(s[StringResource.MULTISTORE_NOTE_LABEL]) },
                placeholder = { Text(s[StringResource.MULTISTORE_NOTE_HINT]) },
                modifier = Modifier.fillMaxWidth(),
                minLines = 2,
                maxLines = 4,
            ...
            Row(
                ...
                Button(onClick = onSubmit, enabled = !isLoading) {
                    Text(if (isLoading) s[StringResource.COMMON_SAVING] else s[StringResource.MULTISTORE_LOG_EVENT])
                }
            }
        }
    }
