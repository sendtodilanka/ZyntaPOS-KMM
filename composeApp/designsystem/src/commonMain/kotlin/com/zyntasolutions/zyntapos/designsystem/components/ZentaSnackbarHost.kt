...
// ─────────────────────────────────────────────────────────────────────────────
// ZentaSnackbarHost — M3 SnackbarHost with SUCCESS/ERROR/INFO variants.
// Uses ZentaSnackbarVisuals to carry variant metadata.
// ─────────────────────────────────────────────────────────────────────────────

/** Semantic variant for [ZentaSnackbarVisuals]. */
 ...
 */
data class ZentaSnackbarVisuals(
...
/**
 * A [SnackbarHost] that renders [ZentaSnackbarVisuals] with leading icons and
 ...
 * Scaffold(
 *     snackbarHost = { ZentaSnackbarHost(hostState) }
 * ) { ... }
 * ```
 *
 * To show a snackbar:
 * ```
 * hostState.showSnackbar(ZentaSnackbarVisuals("Saved", SnackbarVariant.SUCCESS))
 * ```
 *
...
fun ZentaSnackbarHost(
    ...
    SnackbarHost(
        ...
        val (containerColor, contentColor, icon) = when {
            visuals is ZentaSnackbarVisuals && visuals.variant == SnackbarVariant.SUCCESS ->
                Triple(Color(0xFF2E7D32), Color.White, Icons.Default.CheckCircle as ImageVector)

            visuals is ZentaSnackbarVisuals && visuals.variant == SnackbarVariant.ERROR ->
