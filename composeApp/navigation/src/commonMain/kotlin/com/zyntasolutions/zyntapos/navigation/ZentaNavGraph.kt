 ...
 * Examples:
 * - `zyntapos://product/4901234567890` → [ZentaRoute.ProductDetail]
 * - `zyntapos://order/ORD-2025-0042`  → [ZentaRoute.OrderHistory]
 */
const val ZENTA_DEEP_LINK_SCHEME = "zyntapos"

/** Deep link: `zyntapos://product/{productId}` — scans barcode → ProductDetail. */
val deepLinkProduct: NavDeepLink = navDeepLink<ZentaRoute.ProductDetail>(
    basePath = "$ZENTA_DEEP_LINK_SCHEME://product",
)

/** Deep link: `zyntapos://order/{orderId}` — notification tap → OrderHistory. */
val deepLinkOrder: NavDeepLink = navDeepLink<ZentaRoute.OrderHistory>(
 ...
 * to [ZentaRoute.Dashboard], bypassing [ZentaRoute.Login]. The redirect is
 * driven by [LaunchedEffect] so it happens after the initial composition settles.
 *
 * **Back-stack architecture:**
 * ```
 * NavHost (startDestination = ZentaRoute.Login)
 * │
 * ├── authNavGraph
 * │   ├── Login (start)
 * │   └── PinLock
...
fun ZentaNavGraph(
    ...
    NavHost(
        ...
        startDestination = ZentaRoute.Login,
    ) {
